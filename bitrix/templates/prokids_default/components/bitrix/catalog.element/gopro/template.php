    ...
    ?><div class="pictures changegenimage"><?
        ?><div class="pic"><?
            if (isset($arResult['FIRST_PIC_DETAIL']['SRC'])) {
                ?><div class="glass"><?
                    ?><img class="js_picture_glass genimage" src="<?=$arResult['FIRST_PIC_DETAIL']['SRC']?>" alt="<?=$arResult['IPROPERTY_VALUES']['ELEMENT_PREVIEW_PICTURE_FILE_ALT']?>" title="<?=$arResult['FIRST_PIC_DETAIL']['RESIZE']['TITLE']?>" /><?
    ...
    ?><div class="detail-product__info"><?
                                    ...
                                    foreach ($PRODUCT['PRICE_MATRIX']['COLS'] as $typeID => $arType):
                                        ...
                                        <td class="nowrap">
                                            <span class="price old price_pv_<?=$arType['NAME'];?>">
                                                <?php if ($arPrice['DISCOUNT_DIFF']) { echo $arPrice['PRINT_PRICE']; } ?>
                                            </span>
                                        </td>
                                        <?php
            ...
            <div class="soloprice">
                ...
                <?php if ($arParams['USE_PRICE_COUNT']):?>
                    <?php foreach ($PRODUCT['PRICE_MATRIX']['COLS'] as $typeID => $arType): ?>
                        <?php $arPrice = $PRODUCT['PRICE_MATRIX']['MATRIX'][$typeID][0]; ?>
                        <tr>
                            <td>
        ...
        ?><noindex><div class="buy clearfix"><?
            ?><form class="add2basketform js-buyform<?=$arResult['ID']?> js-synchro<?if(!$PRODUCT['CAN_BUY']):?> cantbuy<?endif;?> clearfix" name="add2basketform"><?
                ...
                ?><a rel="nofollow" class="submit add2basket btn1" href="#" title="<?=GetMessage('ADD2BASKET')?>"><i class="icon pngicons"></i><?=GetMessage('CT_BCE_CATALOG_ADD')?></a><?
                ?><a rel="nofollow" class="inbasket btn2" href="<?=$arParams['BASKET_URL']?>" title="<?=GetMessage('INBASKET_TITLE')?>"><i class="icon pngicons"></i><?=GetMessage('INBASKET')?></a><?
                ?><a rel="nofollow" class="go2basket" href="<?=$arParams['BASKET_URL']?>"><?=GetMessage('INBASKET_TITLE')?></a><?
                ?><a rel="nofollow" class="buy1click detail fancyajax fancybox.ajax btn3" href="<?=SITE_DIR?>include/popup/buy1click/" title="<?=GetMessage('BUY1CLICK')?>"><?=GetMessage('BUY1CLICK')?></a><?
                // SUBSCRIBE
                if ($showSubscribeBtn):
            ...
            if ($arParams['USE_FAVORITE']=='Y' || $arParams['USE_CHEAPER']=='Y') {
                ?><div class="favoriteandcheaper"><?
                    ...
                    if ($arParams['USE_CHEAPER'] == 'Y') {
                        ?><div class="cheaper"><?
                            ?><a rel="nofollow" class="cheaper detail fancyajax fancybox.ajax" href="<?=SITE_DIR?>include/popup/cheaper/" title="<?=GetMessage('CHEAPER')?>"><i class="icon pngicons"></i><?=GetMessage('CHEAPER')?></a><?
                        ?></div><?
                    }
                ?></div><?
            }

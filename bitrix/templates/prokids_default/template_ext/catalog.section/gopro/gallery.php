...
if (is_array($arResult['ITEMS']) && count($arResult['ITEMS'])>0) {
	...
	if($arParams['IS_AJAXPAGES']=="Y") {
		...
		foreach($arResult['ITEMS'] as $key1 => $arItem) {
			...
			?><tr class="js-element js-elementid<?=$arItem['ID']?> <?if($haveOffers):?>offers<?else:?>simple<?endif;?><?if( ($inc+1)%2==0 ):?> even<?endif;?>" data-elementid="<?=$arItem['ID']?>" data-detail="<?=$arItem['DETAIL_PAGE_URL']?>" id="<?=$this->GetEditAreaId($arItem["ID"]);?>"><?
				?><td class="free<?
					if( 
						isset($arItem['DAYSARTICLE2']) ||
						isset($PRODUCT['DAYSARTICLE2']) ||
                            ...
                            if (!$haveOffers && $showSubscribeBtn):
                                $APPLICATION->includeComponent('bitrix:catalog.product.subscribe', 'gopro',
                                    array(
                                        ...
                                        'BUTTON_CLASS' => 'add2subscribe btn3 icon',

...
if(is_array($arResult['ITEMS']) && count($arResult['ITEMS'])>0) {
	...
	if($arParams['IS_AJAXPAGES']=="Y") {
		...
		foreach($arResult['ITEMS'] as $key1 => $arItem) {
			...
			?><tr class="js-element js-elementid<?=$arItem['ID']?> <?if($haveOffers):?>offers<?else:?>simple<?endif;?><?if( ($inc+1)%2==0 ):?> even<?endif;?>" data-detail="<?=$arItem['DETAIL_PAGE_URL']?>" data-elementid="<?=$arItem['ID']?>" id="<?=$this->GetEditAreaId($arItem["ID"]);?>"><?
				?><td class="free"><?
					?><span<?
						if( 
							isset($arItem['DAYSARTICLE2']) ||

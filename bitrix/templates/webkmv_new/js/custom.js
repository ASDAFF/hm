...
jQuery(document).ready(function($){
		...
		$('.z_main_tizer_seminar_cl').click(function(){
			$('.z_main_tizer_seminar').hide();
		});

		$(window).scroll(function(){
			if($(window).scrollTop() > 200){
				$('.z_main_tizer_seminar').addClass('z_main_tizer_seminar_small');
			}else{
				$('.z_main_tizer_seminar').removeClass('z_main_tizer_seminar_small');
			}
		});
	...
	$('.scrolltohref').click(function(){
		$.scrollTo($(this).attr('href'), 1400, { offset: { top: -92 }});
		return false;
	});
	...
	$('#home .sub-title h21').funnyText({
        speed: 500,

        borderColor: 'none'
    });
	
	$('#services .main-title h21').funnyText({
        speed: 500,

        borderColor: 'none'
    });
	
    $('#team .main-title h21').funnyText({
        speed: 500,

        borderColor: 'none'
    });
    $('#portfolio .main-title h21').funnyText({
        speed: 500,

        borderColor: 'none'
    });
    $('#blog .main-title h21').funnyText({
        speed: 500,

        borderColor: 'none'
    });
    $('#contact .main-title h21').funnyText({
        speed: 500,

        borderColor: 'none'
    });
	
	$('#page-title h11').funnyText({
        speed: 500,
	...
	$('form[name="frmnewsletter"], form[name="commentform1"], form[name="frmsearch"]').each(function(){
        $(this).validate({
...
function subcommentform(form){
	...
	jQuery.ajax({
			...
			success: function(msg){
				if( msg == 'Y'){
					jQuery('#ourform').html('');
					alert('Спасибо, наши специалисты свяжутся с Вами' + msg);
				};
			}
	});
	return false;
...
function sended(){
    ...
    if(validateit(ajaxForm)){

        var sendeddata = {};


        ajaxForm.find('.submited').each(function(){
            if($(this).val().length > 0){
                var key = $(this).attr('name');
                var addedArray = $(this).val();
                sendeddata[key] = addedArray;
            }
        });

        ajaxForm.find('.submited_radio').each(function(){
        ...
        $.post(
            ...
            { 'your-name': ajaxForm.find("input[name='your-name']").val(),
                'your-email': ajaxForm.find("input[name='your-email']").val(),
                'your-message': ajaxForm.find("textarea[name='your-message']").val()
            },
            function(msg){
                /*console.log(data);*/
                if( msg == 'Y'){
                    jQuery('#ourform').html('');
                    alert('Спасибо, наши специалисты свяжутся с Вами');
                };
               /*$.fancybox.open("Сообщение отправлено");*/
            }
        );
...
function validateit(validform){
    ...
    if(!returned){
        ...
        /*jQuery.fancybox('Вы заполнили не все поля');*/
    }
